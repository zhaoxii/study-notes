...
class NewsTypeApi(Resource):
    ...
    # 更新修改分类名称  修改单条数据， 多条用put
    def patch(self):
        args=update_type_parser.parse_args()
        id=args.get('id')
        new_type_name=args.get('typeName')
        newstype = NewType.query.get(id)
        if newstype:
            newstype.type_name=new_type_name
            db.session.commit()
            data = {
                'status': 200,
                'msg': '修改成功',
                'type': marshal(newstype, types_fields)
            }
    ...
    def delete(self):
        ...
        typeId = args.get('id')
        type_obj = NewType.query.get(typeId)
        if type_obj:
...
news_parser.add_argument('page',type=int,default=1)
...
news_fields={
    ...
    'url':fields.Url(endpoint='news.newsdetail',absolute=True)
}
